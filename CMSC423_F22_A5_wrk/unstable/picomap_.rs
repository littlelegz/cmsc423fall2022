...
fn main() {
    ...
    for (idx, query) in problems.clone().iter().enumerate() {
        ...
        if fmseed.1 == query.query.len() {
            problems[idx].ref_start = fmseed.0[0];
            problems[idx].score = "0";
            problems[idx].cigar = "100=".to_string();
        } else if fmseed.0.len() > 0 {
        ...
        // Calling aligner
            ...
            if result.0 < -10 {
                ...
                result = reference_align(query, reference_slice, -mismatch_pen, -gap_pen);
                if problems[idx].score < result.0 {
                    //println!("Score was: {}. Switching to saseeds: {:?} with new score of {}", problems[idx].score, saseed, result.0);
                    //println!("Reference slice: {}", reference_slice);
...
pub fn simple_accel_wrapper(query: &str, sa: &Vec<usize>, genome: &str) -> (Vec<usize>, u32, u32) {
    ...
    let lower = simple_accel(query, sa, genome);
    //
...
pub fn simple_accel(target: &str, sa: &Vec<usize>, genome: &str) -> (u32, u32) {
    ...
    loop {
        ...
        let new_matched = matched + result.1 - 1;
        if new_matched != matched {
            best_match = middle as u32;
        } 
        matched = max(result_high, result_low);
        
        println!("Current matched count: {} at: {}", matched, sa[middle as usize]);
